...
import { NoteTag } from "@/types/note"; // ВИПРАВЛЕНО: Використовуємо NoteTag
...
export default function NoteForm() {
  ...
  // Zustand store для роботи з чернеткою
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Очищаємо чернетку після успіху
      clearDraft();
      // Повертаємося назад
      router.back();
    },
  ...
  const handleChange = (
    ...
    // Оновлюємо об'єкт draft. 
    // TypeScript автоматично перевірить, чи відповідає value типу NoteTag для поля tag.
    setDraft({
      ...draft,
      [name]: value,
    });
  ...
  const handleCancel = () => {
    // Повертаємось назад, чернетка зберігається в Zustand/LocalStorage
    router.back();
  };
